...
const Index: React.FC<Props> = ({ children, ...props }) => {
  return (
    <button
      {...props}
      className={`overflow-hidden relative mt-8 mx-auto tracking-widest rounded-sm
       px-16 py-4 outline-[1.5px] text-green outline outline-green 
       before:content[''] before:h-full before:w-full 
       before:scale-x-0 before:absolute before:top-0 before:left-0 before:bg-green/25
